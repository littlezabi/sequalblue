...
const num_abr = ['single', 'dual', 'triple', 'quadruple', 'quintuple', 'sextuple']
const item_keys = Object.keys(items)
const remove_features = ['description',"title", "keywords", 'category', 'slug', 'loves', 'views', 'rating', 'original']
</script>
        ...
        {#each Object.keys(value) as val_key}
          {#if value[val_key]}
            <div
              use:viewport
              on:enterViewport={(e) =>
                (e.target.className = "from-right")}
              on:exitViewport={(e) =>
                (e.target.className = "from-right-init")}
            >
              ...
              <span class="product-text">
                {#if item_key == "display"}
                  {#if val_key == "display height"}{value[
                      val_key
                    ]} inches
                  {:else if val_key == "display width"}{value[
                      val_key
                    ]} cm<sup>2</sup>
                  {:else if val_key == "screen-to-body"}{value[
                      val_key
                    ]}% screen ratio
                  {:else if val_key == "screen width"}<span
                    ...
                    >
                  {:else if val_key == "screen height"}{value[
                      val_key
                    ]}px
                  {:else if val_key == "screen ratio"}{value[
                      val_key
                    ]} aspect ratio
                ...
                {:else if item_key === "platform"}
                  {#if val_key == "chipset"}
                    <span class="tag-888321 c2"
                      >{value[val_key]}</span
                    >
                  {:else if val_key == "os"}
                    <span class="tag-888321 c4"
                      >{value[val_key]}</span
                    >
                  {:else}
                    {value[val_key]}
